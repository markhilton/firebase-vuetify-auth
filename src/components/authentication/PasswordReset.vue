			...
			<v-form ref="form" v-model="valid" @submit.prevent="emailPasswordResetLink">
				...
				<div v-if="!success">
					<v-card-text class="mb-0 pb-0">
						<div class="mb-5">
							Enter registered user email address and we will send you a link to reset your
							password.
						</div>
				...
				<v-container v-if="success" class="pa-4 text-center">
					...
					<v-card-text
						>Please check your inbox and follow the instructions in the email to reset your
						account password</v-card-text
					>

					<v-card-actions>
						<v-btn block large depressed color="primary" @click="$emit('showSignInTab')">
							Login
						</v-btn>
					</v-card-actions>
				</v-container>
